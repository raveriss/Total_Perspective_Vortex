...
def write_fake_uv(script_path: Path, body: str) -> None:
...
def run_make_goals(
    ...
    *,
    ...
    fake_uv = tmp_path / "bin" / "fake-uv"
    write_fake_uv(fake_uv, fake_uv_body)
    if fake_sudo_body is not None:
        write_fake_uv(tmp_path / "bin" / "sudo", fake_sudo_body)
    env = os.environ.copy()
...
""",
    ...
    assert "uv should not run" not in combined_output
    assert "make: ***" not in combined_output
