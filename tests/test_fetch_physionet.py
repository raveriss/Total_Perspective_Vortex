...
def test_validate_file_rejects_hash_mismatch(monkeypatch, tmp_path: Path) -> None:
    # Crée un fichier stable pour contrôler le hash calculé
    file_path = tmp_path / "sample.edf"
    # Définit une charge utile contrôlée pour le hachage
    payload = b"content"
    ...
    class SafeHandle:
        ...
        def __init__(self) -> None:
            self.chunks = [payload[:2], payload[2:], b""]
            self.eof_seen = False

        # Signale toute lecture après la fin de fichier pour tuer les mutants
...
def test_load_manifest_opens_manifest_in_text_mode_with_utf8(
    tmp_path: Path, monkeypatch
) -> None:
    # Crée un fichier manifeste réel pour satisfaire exists() et is_dir()
    manifest_path = tmp_path / "manifest.json"
    # Écrit un contenu placeholder car l'ouverture sera interceptée
    manifest_path.write_text("{}", encoding="utf-8")
    # Capture les paramètres exacts passés à Path.open()
    recorded: dict[str, object] = {}

    # Remplace Path.open pour verrouiller mode et encoding attendus
    def open_spy(self: Path, *args: object, **kwargs: object):
        # Archive le chemin utilisé pour vérifier l'appel ciblé
        recorded["self"] = self
        # Archive les arguments positionnels pour verrouiller "r"
        recorded["args"] = args
        # Archive les kwargs pour verrouiller encoding="utf-8"
        recorded["kwargs"] = kwargs

        # Fournit un contexte lisible par json.load sans accès disque
        class Ctx:
            # Retourne un flux texte conforme au JSON attendu
            def __enter__(self) -> StringIO:
                return StringIO(json.dumps({"files": []}))

            # Termine le contexte sans nettoyage spécifique
            def __exit__(self, *_: object) -> None:
                return None

        return Ctx()

    # Intercepte Path.open pour contrôler strictement l'API utilisée
    monkeypatch.setattr(Path, "open", open_spy)
    # Exécute le chargement pour déclencher l'appel open instrumenté
    entries = fetch_physionet.load_manifest(manifest_path)
    # Vérifie que le contenu parsé retourne une liste vide valide
    assert entries == []
    # Vérifie que l'ouverture a ciblé le manifeste demandé
    assert recorded["self"] == manifest_path
    # Verrouille le mode texte explicite attendu
    assert recorded["args"] == ("r",)
    # Verrouille l'encodage exact attendu (casse comprise)
...
def test_load_manifest_defaults_files_to_empty_list_when_missing(
    ...
    # Charge le manifeste pour vérifier le comportement par défaut
    entries = fetch_physionet.load_manifest(manifest_path)
    # Vérifie que l'absence de clé "files" produit une liste vide
    assert entries == []
...
def test_load_manifest_rejects_non_dict_entries(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError) as error:
        fetch_physionet.load_manifest(manifest_path)
    # Verrouille le message pour refuser ValueError(None)
    assert (
        str(error.value)
        == f"{fetch_physionet.ERROR_PREFIX} entrée de manifeste non dictionnaire: bad-entry"
    )


# Calcule un hash stable pour sécuriser la validation SHA-256
def test_compute_sha256_matches_expected(monkeypatch, tmp_path: Path) -> None:
...
def test_validate_file_accepts_matching_metadata(monkeypatch, tmp_path: Path) -> None:
